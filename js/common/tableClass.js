...
sorttable = {
		...
		for ( var i = 0; i < headrow.length; i++) {
			...
			if (!headrow[i].className.match(/\bsorttable_nosort\b/)) { // skip
				...
				dean_addEvent(headrow[i], "click", function(e) {
					...
					// sort based on the sort keys, and then put the rows back
					// in order
					// which is a lot faster because you only do getInnerText
					// once per row
					row_array = [];
	...
	guessType : function(table, column) {
		...
		for ( var i = 0; i < table.tBodies[0].rows.length; i++) {
			...
			if (text != '' && text != null && text != undefined && undefined !='undefined') {
				if (text.match(/^-?[$]?[\d,.]+%?$/)) {
